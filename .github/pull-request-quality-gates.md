...
The Quality assurance workflow starts formatting, linting, type-checking, unit tests with SonarCloud, Chromatic, and delivery-catalog drift validation independently. Unit-test workspaces run with at most three concurrent processes. Each Vitest workspace uses `--no-file-parallelism` in its own `test-ci` script, bounding the job to three test-file processes while preserving serial execution within a workspace. The Preview Environment workflow reports `Preview` for deployment and `Playwright` for the unchanged end-to-end suite; both report a successful not-applicable result for non-runtime changes.
