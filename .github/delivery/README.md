...
After publication succeeds, the planner resolves published, non-prerelease GitHub Releases through their Git tags. Selective planning seeds the affected workspace graph from Releases published at the accepted main commit boundary rather than raw changed paths. Every directly or workspace-dependency-affected Deployment Unit must own a successful Release at that boundary. Delivery-only dependants may use their latest owning-workspace Release reachable from the boundary, and a package tag is never considered for an application workspace.
