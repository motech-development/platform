...
test('replacement Accounts web uses normal delivery and one-to-one hosted notification journeys', async () => {
  ...
  assert.match(
    previewValidation,
    /Build legacy fixture client[\s\S]*Run hosted replacement journeys with legacy event producers[\s\S]*should create a company[\s\S]*should update company settings\|should add client 1\|should add a confirmed sale"[\s\S]*should generate and download report"\s+\\\s+--timeout=60000[\s\S]*BASE_URL="\$DEPLOYMENT_URL"[\s\S]*receives and dismisses the established report-ready notification[\s\S]*should create a company[\s\S]*should update company settings\|should add client 1\|should add a confirmed sale\|should schedule a purchase refund\|should have published the scheduled transaction"[\s\S]*should generate a report"\s+\\\s+--timeout=60000[\s\S]*BASE_URL="\$DEPLOYMENT_URL"[\s\S]*receives and dismisses the established scheduled-transaction notification[\s\S]*grep-invert "receives and dismisses the established \(report-ready\|scheduled-transaction\) notification"/,
  );
  assert.match(
    previewValidation,
