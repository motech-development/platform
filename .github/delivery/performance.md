...
## Manual full delivery and recovery evidence

Two manual full deliveries reused the successfully released main boundary [`db781061`](https://github.com/motech-development/platform/commit/db78106196d2b21634ce78d0c303225e0cd2ef1b). In both [run A, 30088452977](https://github.com/motech-development/platform/actions/runs/30088452977) and [run B, 30088564549](https://github.com/motech-development/platform/actions/runs/30088564549), the `Packages` job checked out that boundary, passed `Validate manual Release boundary` by finding its successful push Release job in run 30084194464, resolved published GitHub Releases, and invoked the Release Plan generator in `full` mode. Both runs produced the exact desired-tag map recorded above, including the three unchanged prerequisites, without publishing new packages.

Before run A, a controlled [Develop Deployment 5587787435](https://api.github.com/repos/motech-development/platform/deployments/5587787435) recorded `failure` for `deploy:accounts-notifications` at the already released `@accounts/notifications@1.4.2` tag. Run A's Develop reconciliation selected Accounts notifications plus its delivery dependants, Accounts reports and Accounts API; every other Develop Deployment Unit was skipped. Accounts notifications created [replacement Deployment 5587812262](https://api.github.com/repos/motech-development/platform/deployments/5587812262), changed from `in_progress` at 11:08:05 UTC to `success` at 11:09:19 UTC, and the two selected dependants also succeeded. Run A completed successfully at 11:12:46 UTC.

Run B was dispatched at 11:08:48 UTC while run A's Accounts notifications mutation was in progress. Its `Packages` job completed at 11:09:20 UTC, but environment concurrency left its Develop setup pending until 11:12:49 UTC, four seconds after run A's final Develop mutation completed. Run B then reread Environment State and skipped every Develop deployment job because run A had repaired the failed unit and its dependants. The production lock behaved independently: run B's production setup started at 11:12:02 UTC, after run A's last production job completed at 11:11:58 UTC. Neither active environment mutation was cancelled, and both full-delivery runs completed successfully.
