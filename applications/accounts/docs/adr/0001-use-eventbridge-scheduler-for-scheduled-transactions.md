...
Failures to synchronise a Transaction change into EventBridge Scheduler will no longer be swallowed. The DynamoDB stream record will be attempted up to five times before being sent to a schedule-synchronisation dead-letter queue.
...
Each dead-letter queue will have a CloudWatch alarm that invokes a Sentry-reporting Lambda when messages are present. Sentry is the operational notification surface, while failed messages remain in their queues for diagnosis and recovery.

Dead-letter recovery is manual after the underlying fault is understood. Publication-processing failures can be redriven to the publication queue; scheduler-delivery failures require the affected schedule to be retried or recreated.
