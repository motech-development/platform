...
Pin **`gpt-6-luna`** and **`high` reasoning effort** (High) for each invocation. Override
...
Confirm `gpt-6-luna`, `high`, and the intended scope in the run header or tool
...
[GPT-6 Luna supports `high` reasoning effort](https://developers.openai.com/api/docs/models/gpt-6-luna).
