...
`,(0,p.jsxs)(t.p,{children:[`Set `,(0,p.jsx)(t.code,{children:`nested`}),` only when the confirmation opens above an existing modal layer, such as a dirty-form confirmation above a drawer. Breeze adds a dimming layer scoped to the nearest open modal surface and centres the confirmation within that task while retaining its own focus scope and alert-dialog semantics. If the parent surface closes while the confirmation remains open, Breeze follows another open modal surface or falls back to the viewport. During that handoff, the active confirmation owns the only backdrop while the exiting parent remains mounted. Do not use `,(0,p.jsx)(t.code,{children:`nested`}),` for a standalone confirmation.`]}),`
...
`,(0,p.jsxs)(t.li,{children:[(0,p.jsx)(t.strong,{children:`External trigger:`}),` set `,(0,p.jsx)(t.code,{children:`triggerless`}),`, `,(0,p.jsx)(t.code,{children:`open`}),`, and `,(0,p.jsx)(t.code,{children:`onOpenChange`}),` when application state opens the confirmation from an action outside the fixed pattern. `,(0,p.jsx)(t.code,{children:`trigger`}),` is then omitted, and focus returns to the previously focused external action after closure.`]}),`
`]}),`
...
`,(0,p.jsxs)(t.ul,{children:[`
`,(0,p.jsx)(t.li,{children:`Enter or Space activates the pattern trigger, an application-owned external trigger, and action buttons.`}),`
...
`,(0,p.jsx)(t.li,{children:`Closing restores focus to the pattern trigger or the previously focused external trigger when it remains mounted.`}),`
